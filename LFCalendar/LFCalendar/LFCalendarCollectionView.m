...
- (instancetype)initWithFrame:(CGRect)frame collectionViewLayout:(UICollectionViewLayout *)layout {
    ...
    if (self) {
        ...
        [self registerClass:[LFCalendarHeaderView class] forSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:@"DAPunchAwardCalendarHeaderView"];
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    LFCalendarCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"DAPunchAwardCalendarCollectionViewCell" forIndexPath:indexPath];
    ...
    LFLogModel *model = [self.punchCardLogDic valueForKey:[NSString stringWithFormat:@"%ld",(long)month]];
    ...
    if (indexPath.row%(42) - weekIndex + 1 >= 0&&indexPath.row%(42) - weekIndex + 1 < totalDay) {
        ...
        __block LFEverydayModel *todayModel;
        [model.punchCardEverydayArr enumerateObjectsUsingBlock:^(LFEverydayModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            NSString *dateStr = [obj.dateStr substringFromIndex:6];
            if (dateStr.intValue == day.intValue) {
                todayModel = obj;
                *stop = YES;
            }else {
                todayModel = nil;
        ...
        if (indexPath.row%7 == 0) {
            todayModel.isLineFirst = YES;
        }
        
        if (indexPath.row%7 == 6) {
            todayModel.isLineLast = YES;
        }
        
        cell.model = todayModel;
